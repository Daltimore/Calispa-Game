        ...
        <div class="container hamilton--header--text">
          ...
          <div class="hamilton--lyrics--text">
            ...
            <div class="hamilton--answers">
              <a v-bind:class="{ 'wronganswer': hasAnswered && !item.correct, 'correctanswer': hasAnswered && item.correct}" @click="checkAnswer(item)" v-for="(item, index) in options" :key="index">{{item.name}}</a>
            </div>
          </div>
        </div>
      </div>
